...
xcopy "%~dp0\ui" "%~dp0\dist\ui" /i /e /y /c
...
powershell.exe -nologo -noprofile -command "& { Compress-Archive -Force -Path '%~dp0\dist\ui', '%~dp0\dist\script_hooks', '%~dp0\dist\scripts' -DestinationPath '%~dp0\dist\%modname%_%version%.zip' }"
